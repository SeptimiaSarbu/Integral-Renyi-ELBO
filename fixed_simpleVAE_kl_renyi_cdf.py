...
def gaussian_cdf_decoder(hidden_decoder, x_samples):
    W_decoder_hidden_reconstr_mu_kl = weight_variable([hidden_decoder_dim, input_dim],"W_decoder_hidden_reconstr_mu_kl")
    b_decoder_hidden_reconstr_mu_kl = bias_variable([input_dim],"b_decoder_hidden_reconstr_mu_kl")

    W_decoder_hidden_reconstr_logvar_kl = weight_variable([hidden_decoder_dim, input_dim],"W_decoder_hidden_reconstr_logvar_kl")
    b_decoder_hidden_reconstr_logvar_kl = bias_variable([input_dim],"b_decoder_hidden_reconstr_logvar_kl")

    #
    param_mul = tf.constant(1.0, dtype=tf_type)
    mu_decoder = tf.sigmoid(tf.multiply(param_mul,tf.matmul(hidden_decoder, W_decoder_hidden_reconstr_mu_kl) + b_decoder_hidden_reconstr_mu_kl))
    ...
    logvar_decoder = tf.matmul(hidden_decoder, W_decoder_hidden_reconstr_logvar_kl) + b_decoder_hidden_reconstr_logvar_kl
    # std_decoder = tf.fill(tf.shape(mu_decoder), 1e-1)

    std_decoder = tf.exp(0.5 * logvar_decoder)+1e-5#std_decoder = tf.exp(0.5 * logvar_decoder)

    # std_decoder = tf.sigmoid(logvar_decoder) + 1e-5
    # std_decoder = tf.exp(-(tf.pow(logvar_decoder, 2))) + 1e-5
    # std_decoder = tf.nn.softplus(logvar_decoder) + 0e-5

    #param=tf.constant((64.0)**1,dtype=tf_type)#32 is ok too
    #offset_std=tf.divide(1,tf.sqrt(param))+1e-5
    #std_decoder = offset_std + tf.divide(logvar_decoder,tf.sqrt(offset+tf.multiply(param,tf.pow(logvar_decoder,2))))

    # pdb.set_trace()

    #cdf_poz = Normal(mu_decoder, std_decoder).cdf(x_samples + 2e-1)
    #cdf_neg = Normal(mu_decoder, std_decoder).cdf(x_samples - 2e-1)

    cdf_poz = Normal(mu_decoder, std_decoder).cdf(x_samples + 0.5e-1)#2
    cdf_neg = Normal(mu_decoder, std_decoder).cdf(x_samples - 0.5e-1)#2


    #cdf_poz = Normal(mu_decoder, std_decoder).cdf(3 * x_samples / 2 + 2e-3)
    #cdf_neg = Normal(mu_decoder, std_decoder).cdf(x_samples / 2 + 1e-3)

    #If cdf_poz==cdf_neg, then skip these x_samples from the training
    #log_cdf_components=tf.cond(tf.equal(cdf_poz,cdf_neg), lambda: 1.0, lambda: tf.log(cdf_poz - cdf_neg))
    indices=tf.where(tf.equal(cdf_poz,cdf_neg))

    #cdf_poz_check = Normal(mu_decoder, std_decoder).cdf(x_samples + 0.5e-1)

    log_cdf_components = tf.log(cdf_poz - cdf_neg+1e-8)
    #log_cdf_components = tf.log(cdf_poz)

    log_cdf_reconstruct = tf.reduce_sum(log_cdf_components, 1)
    ...
    # log_pdfs_reconstruct = MultivariateNormalDiag(mu_decoder, std_decoder)._log_prob(x_samples)
    # return cdf_poz, cdf_neg, log_cdf_reconstruct, log_pdfs_reconstruct, std_decoder, mu_decoder
    return cdf_neg,cdf_poz, log_cdf_reconstruct, std_decoder, mu_decoder,indices,x_samples,log_pxz, pxz, log_cdf2_pxz
...
########################################################################################################################
#for IELBO
cdf_neg, cdf_poz, log_pdfs_reconstruct, std_decoder, mu_decoder, indices, x_samples,log_pxz, pxz, log_cdf2_pxz = gaussian_cdf_decoder(
    hidden_decoder, x_samples)

KLD = -0.5 * tf.reduce_sum(1 + logvar_encoder - tf.pow(mu_encoder, 2) - tf.exp(logvar_encoder), reduction_indices=1)

# mean over number of samples from q(z|x)
reconstruction = tf.reduce_mean(tf.reshape(log_pdfs_reconstruct, [-1, n_samples]), axis=1)
...
# average bound over the batch size
# loss = tf.reduce_mean(-reconstruction + KLD)

loss_reconstruction = tf.reduce_mean(-reconstruction)
loss_KLD = tf.reduce_mean(KLD)
    ...
    for N in range(1, n_epochs+1, 1):
        for step in range(1, n_steps+1,1):
            ...
            if ( N % 100 == 0) & (step % 550 == 0):#if (NL+N  == 9) & (step % 1 == 0):#(stepL+step > 899): #| (step % 50 == 0): (NL+N == 9) &
                ...
                # #END for IRELBO
                # ########################################################################################################################
                print("Epoch {0} Step {1} | Loss_train: {2} ".format(NL + N, stepL + step, train_loss))
                ...
                plt.suptitle("Reconstructed test MNIST images: Integral ELBO", fontsize=40)
                fig2.savefig('Reconstructed_test_MNIST_with_IELBO.png')
                ...
                plt.suptitle("Generated test MNIST images: Integral ELBO",
                    fontsize=40)
                fig3.savefig('Generated_test_MNIST_with_IELBO.png')
                #plt.show()
